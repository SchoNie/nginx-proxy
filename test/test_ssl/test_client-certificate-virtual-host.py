...
def test_client_certificate_virtual_host_is_enforced(docker_compose, nginxproxy):
    """
    Test connection to a website with mTLS enabled without providing a client certificate.
    """
    r = nginxproxy.get("https://mtls-enabled.nginx-proxy.tld/port")
    assert r.status_code == 400
    assert "400 No required SSL certificate was sent" in r.text

def test_client_certificate_virtual_host_is_authenticated(docker_compose, nginxproxy, clientcerts):
    """
    Test connection to a website with mTLS enabled providing a valid client certificate.
    """
    r = nginxproxy.get("https://mtls-enabled.nginx-proxy.tld/port", cert=(clientcerts["client_cert"], clientcerts["client_key"]))
    assert r.status_code == 200
    assert "answer from port 84\n" in r.text

def test_client_certificate_virtual_host_is_revoked_crl(docker_compose, nginxproxy, clientcerts):
    """
    Test connection to a website with mTLS enabled providing a revoked client certificate on the CRL.
    """
    r = nginxproxy.get("https://mtls-enabled.nginx-proxy.tld/port", cert=(clientcerts["invalid_client_cert"], clientcerts["invalid_client_key"]))
    assert r.status_code == 400
    assert "400 The SSL certificate error" in r.text

def test_client_certificate_virtual_host_is_optional(docker_compose, nginxproxy, clientcerts):
    """
    ...
    """
    r = nginxproxy.get("https://mtls-optional.nginx-proxy.tld/port")
    assert r.status_code == 200
